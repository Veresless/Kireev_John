...
void Game::pacManEat()
{
	...
	if (Position.first % Size == 0 && Position.second % Size == 0)
	{
		...
		if (Field_.getSpriteTypeAt(Position.first / Size, Position.second / Size) == SpriteType::TEnergiser)
		{
			...
			if (Level_ < 19)
			{
				EnergiserTime_ = GetTickCount();
				Field_.changeCastMode(CastMode::Fear);
				Field_.fearCast();
				CastDieCount_ = 1;
			}
			Field_.setEmptyAt(Position.first / Size, Position.second / Size);
...
void Game::modeTimer()
{
	...
	int FourthWaveRetreat = ((Level_ < 5) ? 5000 : 1) + ThirdWaveAttack;
	...
	while (Live_)
	{
		...
		if (Mode == CastMode::Fear)
		{
			...
			if (GetTickCount64() - EnergiserTime_ >= FearTime)
			{
				Field_.normalCast();
				Sleep(DeltaTime);
				Field_.changeCastMode(CastMode::Attack);
			}
		}
		else if (GetTickCount64() - StartTime >= FourthWaveRetreat && Mode != CastMode::Retreat) Field_.changeCastMode(CastMode::Retreat);
		else if (GetTickCount64() - StartTime >= ThirdWaveAttack && Mode != CastMode::Attack) Field_.changeCastMode(CastMode::Attack);
		else if (GetTickCount64() - StartTime >= ThirdWaveRetreat && Mode != CastMode::Retreat) Field_.changeCastMode(CastMode::Retreat);
		else if (GetTickCount64() - StartTime >= SecondWaveAttack && Mode != CastMode::Attack) Field_.changeCastMode(CastMode::Attack);
		else if (GetTickCount64() - StartTime >= SecondWaveRetreat && Mode != CastMode::Retreat) Field_.changeCastMode(CastMode::Retreat);
		else if (GetTickCount64() - StartTime >= FirstWaveAttack && Mode != CastMode::Attack) Field_.changeCastMode(CastMode::Attack);
		else if (GetTickCount64() - StartTime >= FirstWaveRetreat && Mode != CastMode::Retreat) Field_.changeCastMode(CastMode::Retreat);
