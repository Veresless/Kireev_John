...
bool Game::checkPosition(const std::shared_ptr<PacMan> pacMan, const std::shared_ptr<Gost> gost) const
{
...
void Game::modeTimer(const WaveTime& waveTime, ULONGLONG& startTime )
{
		...
		if (true != field_.pinkyReady && GetTickCount() - startTime >= getPinkyStartTime())
		{
			field_.setPinkyReady();
		}
		if (true != field_.inkyReady && countEatenPoints_ >= getInkyPointCountCondition() &&
			GetTickCount() - startTime >= getInkyStartTime())
		{
			field_.setInkyReady();
		}
		if (true != field_.clydeReady && countEatenPoints_ >= getClydePointCountCondition() &&
			GetTickCount() - startTime >= getClydeStartTime())
		{
			field_.setClydeReady();
		}
		if (mode == GostMode::FEAR)
		{
			...
			if (GetTickCount() - energiserTime_ >= waveTime.fearTime)
			{
				field_.normalGost();
			}
		}
		else
		{
			...
			if (GetTickCount() - startTime >= waveTime.fourthWaveRetreat)
			{
				NewCastMode = GostMode::ATTACK;
			}
			else if (GetTickCount() - startTime >= waveTime.thirdWaveAttack)
			{
				NewCastMode = GostMode::RETREAT;
			}
			else if (GetTickCount() - startTime >= waveTime.thirdWaveRetreat)
			{
				NewCastMode = GostMode::ATTACK;
			}
			else if (GetTickCount() - startTime >= waveTime.secondWaveAttack)
			{
				NewCastMode = GostMode::RETREAT;
			}
			else if (GetTickCount() - startTime >= waveTime.secondWaveRetreat)
			{
				NewCastMode = GostMode::ATTACK;
			}
			else if (GetTickCount() - startTime >= waveTime.firstWaveAttack)
			{
				NewCastMode = GostMode::RETREAT;
			}
			else if (GetTickCount() - startTime >= waveTime.firstWaveRetreat)
			{
				NewCastMode = GostMode::ATTACK;
			}
			if (NewCastMode != mode)
